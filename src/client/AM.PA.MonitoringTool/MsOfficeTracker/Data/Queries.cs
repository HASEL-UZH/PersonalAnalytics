    ...
    public class Queries
    {
        internal static void CreateMsTrackerTables()
        {
            try
            {
                ...
                Database.GetInstance().ExecuteDefaultQuery("CREATE TABLE IF NOT EXISTS " + Settings.EmailsTable + " (id INTEGER PRIMARY KEY, timestamp TEXT, time TEXT, inbox INTEGER, inboxUnread INTEGER, sent INTEGER, received INTEGER, receivedUnread INTEGER);");
            }
            catch (Exception e)
            {
        ...
        internal static Tuple<long, long, long, long, int> CreateEmailsSnapshot(DateTime date)
        {
            try
            {
                ...
                SaveEmailsSnapshot(date, inbox, unreadInbox, sent, received, unreadReceived);
        ...
        internal static void SaveEmailsSnapshot(DateTime date, long inbox, long unreadInbox, long sent, long received, int unreadReceived)
        {
            Database.GetInstance().ExecuteDefaultQuery("INSERT INTO " + Settings.EmailsTable + " (timestamp, time, inbox, inboxUnread, sent, received, receivedUnread) VALUES (strftime('%Y-%m-%d %H:%M:%f', 'now', 'localtime'), " +
                ...
                Database.GetInstance().Q(received) + ", " + Database.GetInstance().Q(unreadReceived) + ");");
        }
        ...
        /// </summary>
        /// <returns>true if yes, false otherwise</returns>
        internal static bool HasEmailsEntriesForDate(DateTimeOffset date)
        {
            try
            {
                var query = "SELECT EXISTS( "
