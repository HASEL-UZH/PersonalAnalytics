...
| `dataExportFormat`       | Defines the format of how the data export file that is created. Options are: `ExportAsSqlite `, `ExportAsZippedSqlite`, `ExportAsZippedJson` and `ExportToDDL`. Contact ameyer@ifi.uzh.ch to use `ExportToDDL`.                                                                                                                                                                                                                                                                                                                                                       |                 | `ExportAsZippedSqlite`        |
...
| `enableRetrospection`     | Whether the Retrospection feature is enabled. If enabled, a daily activity summary (timeline chart and insights) is shown at the end of the workday, a "Retrospection" entry is added to the tray context menu, and a toggle to disable automatic opening is shown under Settings > Self-Reflection. Participants can also manually open it from the context menu. |                 | `true`        |

### Tracker Configuration (edit `trackers` in `study.config.ts`)
| Parameter            | Description                                                                                                                                                                                                                                                              | Change Required | Default Value |
|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|---------------|
| `enabledWorkHours`   | Whether trackers can use participant-defined active work hours. When enabled, participants can set or disable their work hours under 'Settings > Active Times'. Experience sampling and daily surveys use these active times where applicable. By default, work hours are set from 8 AM to 5 PM on weekdays. |                 | `true`        |

#### WindowsActivityTracker
Records which applications the participant uses, by regularly checking the window that is currently in the foreground and storing its process name, window title and, on macOS, the URL of the active browser tab. Each window activity is also categorized into an activity type (e.g. `DevCode`, `Email`, `PlannedMeeting`), based on the application and window title (see the [activity mappings](https://github.com/HASEL-UZH/PA.WindowsActivityTracker/blob/main/typescript/src/mappings/activityMappings.ts) of the PA.WindowsActivityTracker).
...
#### UserInputTracker
Records how much the participant interacts with their computer, by storing aggregated counts of keystrokes, mouse clicks, pixels moved and pixels scrolled per interval, without ever storing the actual keys or text that were typed.
...
| `collectKeyDetails` | Whether keystrokes should be categorized (`keysLetter`, `keysNumber`, `keysNavigate`, etc.; see [full list](https://github.com/HASEL-UZH/PA.UserInputTracker/issues/10)) in addition to total keystrokes. Keep disabled unless your study needs this higher granularity.                                                                                                              |                 | `false`       |

#### ExperienceSamplingTracker
Complements the automatically collected data with self-reported data, by regularly prompting the participant with a small pop-up to answer one or several questions about their current work (e.g. their perceived productivity).
...
| `questions`             | An array of questions that the participant should self-reflect on. You can define one or multiple questions within the array. By default, if the array consists of multiple questions, one question will be randomly selected for each prompt. See [Question format](#question-format) for the fields each question type requires.                                                                                                                                                                                                                                                          | ✅               |                           |
| `showAllQuestionsTogether` | When set to `true`, the experience sampling popup shows all configured questions stacked in their defined order and enables saving once at least one answer was given. When set to `false`, one configured question is randomly selected for each prompt.                                                                                                                                                                                                                                           |                 | `false`                   |
| `allowUserToDisable`         | Allows participants to disable the self-reports. When set to `false`, the Self-Reporting toggle is hidden and user preferences to disable will be ignored.                                                                                                                                                                                                                                                                                                                                        |                 | `true`                    |
| `allowUserToChangeInterval`  | Allows participants to change the interval for periodic self-reports. When set to `false`, the interval dropdown is hidden and any user-specified interval is ignored.                                                                                                                                                                                                                                                                                                                             |                 | `true`                    |
| `userDefinedInterval_h`      | Options for the participant to choose the periodic interval (in hours) when `allowUserToChangeInterval` is `true`. Provide a list of floats, e.g., `[0.5, 1, 2, 3]` (meaning 30 min, 1 h, 2 h, 3 h). If empty, the dropdown won’t be shown.                                                                                                                                                                                                                                                       |                 | `[]`                      |

#### DailySurveyTracker
Asks the participant a set of questions once at the start and/or the end of their workday, scheduled relative to their configured work hours instead of at a fixed interval.

| Parameter                       | Description                                                                                                                                                                                                                                                                                                                                                                               | Change Required | Default Value |
|---------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|---------------|
| `enabled`                       | Whether the DailySurveyTracker is enabled. If enabled, the DailySurveyTracker will prompt the participant with the configured morning and/or evening surveys relative to their work hours.                                                                                                                                                                                                |                 | `false`       |
| `surveys`                       | An array of survey configurations. Each entry defines a survey that is triggered at a specific time relative to the participant’s work hours (e.g. a morning survey and an evening survey).                                                                                                                                                                                               | ✅               |               |
| `surveys[].samplingType`        | When the survey should be triggered. `’morning’` triggers relative to the start of work hours, `’evening’` triggers relative to the end of work hours.                                                                                                                                                                                                                                   | ✅               |               |
| `surveys[].delayInMinutes`      | The offset in minutes from the work hour boundary. Positive values delay the survey (e.g. `5` means 5 minutes after work start), negative values trigger it earlier (e.g. `-30` means 30 minutes before work end).                                                                                                                                                                       | ✅               |               |
...
| `surveys[].questions`           | An array of questions to present in the survey. All questions are shown in order. Questions use the same format as the `ExperienceSamplingTracker` questions and support the `answerType` values `'LikertScale'`, `'SingleChoice'`, `'MultiChoice'` and `'TextResponse'`. See [Question format](#question-format) for the fields each question type requires.                                | ✅               |               |
...
#### Question format
Questions are configured the same way for the `ExperienceSamplingTracker` (`questions`) and for the `DailySurveyTracker` (`surveys[].questions`). Every question is an object with a `question` text and an `answerType`; which further fields apply (and which type they have) depends on the `answerType`. All of them are required, there are no optional question fields. See `ExperienceSamplingQuestion` in [StudyConfiguration.ts](../src/electron/shared/StudyConfiguration.ts) for the authoritative definition.

| Field             | Applies to                     | Type                                                                 | Required | Description                                                                                                                                                                                             |
|-------------------|--------------------------------|----------------------------------------------------------------------|----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `question`        | all                            | `string`                                                             | ✅        | The question text that is shown to the participant.                                                                                                                                                     |
| `answerType`      | all                            | `'LikertScale' \| 'TextResponse' \| 'SingleChoice' \| 'MultiChoice'` | ✅        | How the question is rendered, and which of the fields below apply.                                                                                                                                      |
| `scale`           | `LikertScale`                  | `number`                                                             | ✅        | The number of items of the Likert-scale. Per the definition, it should be an odd number, ideally between 3-9. Defined per question, so questions can use different scales.                               |
| `responseOptions` | `LikertScale`                  | `string[]`                                                           | ✅        | Either two labels displayed on the left and right (e.g. `['strongly disagree', 'strongly agree']`) or three labels displayed on the left, in the middle and right (e.g. `['strongly disagree', 'neutral', 'strongly agree']`). |
| `responseOptions` | `SingleChoice`, `MultiChoice`  | `string[]`                                                           | ✅        | The options to choose from. `SingleChoice` allows exactly one selection, `MultiChoice` any number.                                                                                                      |
| `responseOptions` | `TextResponse`                 | `'singleLine' \| 'multiLine'`                                        | ✅        | A string, **not** an array: `'singleLine'` renders a text input, `'multiLine'` a textarea. Passing an array does not fail loudly, participants always get a textarea instead.                            |
| `maxLength`       | `TextResponse`                 | `number`                                                             | ✅        | The maximum number of characters that can be entered. Also shown in the character counter below the input (e.g. `12/100`).                                                                               |

Commented-out examples for each `answerType` are included in [study.config.ts](../src/electron/shared/study.config.ts).
...
Make sure you use Node.js 24.15.0 or newer within Node.js 24. You can install the dependencies by running the following command in the root directory of the project:
```bash
cd src/electron
npm install
```
